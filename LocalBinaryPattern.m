...
%    Revision: 1.01.20   Date: 30/05/2019
%
...
for i = 2:m+1
    for j = 2:n+1
        ...
        n0 = sum(w .* neighbor);
        n1 = sum(w .* (circshift(neighbor,-1, 2)));
        n2 = sum(w .* (circshift(neighbor,-2, 2)));
        n3 = sum(w .* (circshift(neighbor,-3, 2)));
        n4 = sum(w .* (circshift(neighbor,-4, 2)));
        n5 = sum(w .* (circshift(neighbor,-5, 2)));
        n6 = sum(w .* (circshift(neighbor,-6, 2)));
        n7 = sum(w .* (circshift(neighbor,-7, 2)));
